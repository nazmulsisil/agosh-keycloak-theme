			...
			<div class="mt-10">
				<form class="space-y-6" id="kc-form-login" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
					...
					<div>
						...
						<button style="background: #5A3399" tabindex="4" type="submit" name="login" id="kc-login" class="w-full flex justify-center py-2 px-4 border-4 uppercase tracking-button text-button font-semibold text-white bg-trigo-blue border-trigo-blue hover:text-trigo-blue hover:bg-white">
							${msg("doLogIn")}
						</button>
					</div>
				</form>
